...
6. Если current record содержит `master-execution`, вызови deterministic controller: continuation
   переиспользует track/worktree, independent writer изолируется, а единственный ready slice после
   checkpoint/evidence автоматически продолжает master. Не останавливайся из-за commit и не
   спрашивай merge до integration/finalization boundary; canonical stop condition создаёт handoff.
7. Назначь непересекающиеся области файлов минимально необходимому числу агентов с правом записи.
8. До параллельной реализации явно зафиксируй контракты базы данных, API и интерфейсов.
9. Сначала запусти самые узкие релевантные тесты, затем необходимые integration/component и concrete end-to-end проверки этапа.
