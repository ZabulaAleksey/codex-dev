...
Новый дополнительный Markdown без канонической роли маршрутизируется в `docs/notes/<topic>.md`. Это policy-level guard: существующие файлы не перемещаются по имени, потому что безопасный перенос требует semantic/link audit. Внешняя схема Eraser является derived projection этой карты и не заменяет repository source of truth.
