...
- NEXT: implement and verify `DEV-SEP-D` automation detector and promotion lifecycle, then
  automatically select the unique dependency-ready `DEV-SEP-E`. A checkpoint is not a stop
  condition; merge/push/runtime install remain outside automatic execution.
...
- Status: `running`; Lifecycle: `implementation`; predecessor `DEV-SEP-C` verified at `f100e1b`.
