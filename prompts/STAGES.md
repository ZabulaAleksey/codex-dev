...
- NEXT: implement and verify `DEV-SEP-C` traceability and placement validation, then automatically
  select the unique dependency-ready `DEV-SEP-D`. A checkpoint is not a stop condition;
  merge/push/runtime install remain outside automatic execution.
...
- Status: `running`; Lifecycle: `implementation`; predecessor `DEV-SEP-B` verified at `6e9a76a`.
