...
Статус: SUPERSEDED; текущая installation boundary задана `source-installed-layer.spec.md`
Версия: 1.0
Дата: 2026-08-20

## 1. Цель

Историческая цель этапа — устранить разрывы между прежним каноном и активным пользовательским
слоем. Утверждение ниже о совмещённом Git/runtime root сохранено только как audit trail; текущая
архитектура использует отдельный canonical source и installed runtime layer.
...
Managed-файлы `AGENTS.md`, agents, hooks и rules должны находиться непосредственно в `~/.codex`. Локальные полезные расширения сначала переносятся в этот канонический источник, а не теряются при синхронизации.
